...
for filename in os.listdir(DATA_DIR):
    if filename.endswith('.json'):
        with open(os.path.join(DATA_DIR, filename), 'r') as f:
            crossword_datasets[filename] = json.load(f)
# Endpoint to serve crossword data from data.json
@app.route('/api/crossword-data', methods=['GET'])
def get_crossword_data():
    try:
        return _corsify_actual_response(jsonify(crossword_datasets))
    except Exception as e:
        logging.error(f"Error loading crossword data: {str(e)}", exc_info=True)
        return jsonify({"error": "Failed to load crossword data", "details": str(e)}), 500
