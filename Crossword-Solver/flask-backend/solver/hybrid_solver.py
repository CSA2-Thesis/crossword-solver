...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('hybrid_solver.log')
    ]
)
logger = logging.getLogger(__name__)

class HybridCrosswordSolver:
    def __init__(self, grid: List[List[str]], clues: Dict[str, List[Dict]]):
        logger.info("Initializing HybridCrosswordSolver")
    ...
    def _heuristic(self, slot: Dict, word: str) -> int:
        score = 0
    ...
    def _get_candidates_for_slot(self, slot: Dict) -> List[Tuple[str, int]]:
        ...
        if exact_match and len(exact_match['word']) == slot['length']:
            ...
            if self._is_valid_placement(slot, word):
                return [(word, self._heuristic(slot, word))]
        ...
        for word_info in possible_words:
            ...
            if self._is_valid_placement(slot, word):
                score = self._heuristic(slot, word)
    ...
    def _check_perpendicular_constraints(self, slot: Dict, word: str) -> bool:
        """Check if word fits with existing perpendicular words."""
        slot_key = (slot['number'], slot['direction'])
    ...
    def solve(self) -> Dict:
        ...
        success = self._hybrid_dfs(slot_candidates)
    ...
    def _hybrid_dfs(self, slot_candidates: List[Tuple[Dict, List[Tuple[str, int]]]]) -> bool:
        """Recursive hybrid solver with heuristic-based candidate ordering."""
        if not slot_candidates:
            return True
        
        slot, candidates = slot_candidates[0]
        remaining_slots = slot_candidates[1:]
        
        candidates.sort(key=lambda x: -x[1])
        logger.debug(f"Processing slot {slot['number']} {slot['direction']} with {len(candidates)} candidates")
        ...
        for word, score in candidates:
            ...
            if self._hybrid_dfs(remaining_slots):
                return True
...
def solve_with_hybrid(grid: List[List[str]], clues: Dict[str, List[Dict]]) -> Dict:
    solver = HybridCrosswordSolver(grid, clues)
    return solver.solve()